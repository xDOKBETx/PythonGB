...
logging.basicConfig(level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(funcName)s - Аргументы: %(call_args)s - Результат: %(result)s',
                    datefmt='%Y-%m-%d %H:%M:%S', encoding='utf-8')

# Создаем объект логгера и добавляем обработчик для записи в файл
logger = logging.getLogger('function_log')
file_handler = logging.FileHandler('function_log.txt', mode='w', encoding='utf-8')
file_handler.setFormatter(
    logging.Formatter('%(asctime)s - %(levelname)s - %(funcName)s - Аргументы: %(call_args)s - Результат: %(result)s'))
...
def log_to_file(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Преобразование аргументов вызова в строку
        call_args = ', '.join([repr(arg) for arg in args])

        # Логирование параметров и результата функции
        logger.info('', extra={'call_args': call_args, 'result': result})
